...
function economySection(data) {
  ...
  let content = '';
  // Third metric is Silver by default, but the daily often swaps in Brent Crude
  // (oil_label + wti). Fall back gracefully so it never renders "Silver: undefined".
  const thirdLabel = e.oil_label || 'Silver';
  const thirdVal = e.wti || e.silver || '';
  content += p(`${b('10-Year Treasury:')} ${e.us10year} &nbsp;|&nbsp; ${b('Gold:')} ${e.gold} &nbsp;|&nbsp; ${b(thirdLabel + ':')} ${thirdVal}`);
