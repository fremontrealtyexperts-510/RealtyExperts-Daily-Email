...
function economySection(data) {
  ...
  // Optional Market Briefs graphics (e.g. the gold "Looking Dull" chart, or the
  // "10 Most Valuable Companies" + "Top 5 Economies by GDP" infographics), shown in
  // the broadcast post as well as the email. Use `feature_images` (array) for several,
  // or `feature_image` (single) for one. Each clickable -> full size.
  const featImgs = Array.isArray(e.feature_images) ? e.feature_images
                 : (e.feature_image ? [e.feature_image] : []);
  featImgs.filter(fi => fi && fi.url).forEach(fi => {
    content += `<a href="${fi.url}" target="_blank" rel="noopener noreferrer" style="display:block;text-decoration:none;cursor:zoom-in;margin:8px 0 4px;">
    <img src="${fi.url}" alt="${fi.alt || ''}" style="width:100%;display:block;border-radius:10px;border:1px solid #e2e8f0;" />
  </a>`;
    if (fi.caption) content += `<p style="color:#475569;font-size:13.5px;font-style:italic;line-height:1.6;margin:10px 0 4px;">${fi.caption}</p>`;
    if (fi.source) content += `<p style="color:#94a3b8;font-size:11.5px;margin:0 0 6px;">${fi.source}</p>`;
  });
