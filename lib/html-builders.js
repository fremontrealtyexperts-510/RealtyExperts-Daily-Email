 ...
 * Meridian (paper/ink/gold) building blocks for the Agent Hub post + legacy card builders.
 */
...
function mCell(label, value) {
  ...
  return `<div style="box-sizing:border-box;flex:1 1 120px;min-width:116px;border:1px solid ${M.hair};border-radius:12px;padding:12px 13px;background:${M.paper};">
...
function mCard(num, label, accent, inner) {
  if (!inner) return '';
  return `<div style="background:#FFFFFF;border:1px solid ${M.hair};border-radius:14px;padding:18px 18px 16px;margin-bottom:16px;">
  <div style="display:flex;align-items:baseline;gap:9px;margin-bottom:14px;">
    <span style="font-family:${M_SERIF};font-size:19px;font-weight:600;color:${accent};line-height:1;">${num}</span>
    <span style="font-family:${M_SANS};font-size:11px;letter-spacing:.18em;color:${M.ink};font-weight:700;">${label}</span>
    <span style="flex:1 1 auto;border-top:1px solid ${M.hair};align-self:center;height:0;"></span>
  </div>
  ${inner}
</div>`;
}
// Market pulse strip: the day's headline numbers in one glance, derived from the
// same JSON fields the sections already use (no new data required).
function mPulse(data) {
  const re = data.real_estate || {}, s = data.stocks || {}, e = data.economy || {}, c = data.crypto || {};
  const items = [
    ['30-YR', re.rate_30year],
    ['S&amp;P 500', s.sp500],
    ['10-YR', e.us10year],
    ['GOLD', e.gold],
    ['BTC', c.btc],
  ...
  // flex:1 1 0 + min-width:0 pins all five metrics onto ONE row on any phone;
  // the serif numerals are sized so the widest value still fits a 320px screen.
  const cell = ([label, value]) => {
    const { main, change } = mSplit(value);
    const d = change ? mDir(change) : null;
    return `<div style="flex:1 1 0;min-width:0;text-align:center;padding:1px 0;">
    <div style="font-family:${M_SANS};font-size:8.5px;letter-spacing:.06em;color:${M.soft};font-weight:600;white-space:nowrap;">${label}</div>
    <div style="font-family:${M_SERIF};font-size:15px;font-weight:600;color:${M.ink};margin-top:2px;white-space:nowrap;">${main}</div>
    ${d ? `<div style="font-family:${M_SANS};font-size:9.5px;font-weight:700;color:${d.color};white-space:nowrap;">${d.arrow ? d.arrow + ' ' : ''}${change}</div>` : ''}
  </div>`;
  };
  return `<div style="display:flex;gap:4px;border-top:1px solid ${M.hair};padding:12px 0 10px;">${items.map(cell).join('')}</div>`;
}
function mHeader(data, emailUrl) {
  ...
  return `<div style="text-align:center;padding:4px 0 0;border-bottom:1px solid ${M.hair};margin-bottom:18px;">
  <div style="font-family:${M_SANS};font-size:10px;text-transform:uppercase;letter-spacing:.22em;color:${M.goldDark};font-weight:700;">REALTY EXPERTS®</div>
  <div style="font-family:${M_SERIF};font-size:27px;font-weight:600;color:${M.ink};margin:8px 0 7px;">Daily Market Glance</div>
  <div style="display:flex;align-items:center;gap:12px;max-width:340px;margin:0 auto;">
    <span style="flex:1 1 auto;border-top:1px solid ${M.gold};height:0;"></span>
    <span style="font-family:${M_SANS};font-size:10.5px;letter-spacing:.14em;color:${M.soft};font-weight:600;white-space:nowrap;">${displayDate.toUpperCase()}</span>
    <span style="flex:1 1 auto;border-top:1px solid ${M.gold};height:0;"></span>
  </div>
  <div style="margin:13px 0 15px;"><a href="${emailUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block;border:1px solid ${M.gold};color:${M.goldDark};text-decoration:none;font-family:${M_SANS};font-size:12px;font-weight:600;padding:8px 18px;border-radius:40px;">Open Email in Browser</a></div>
  ${mPulse(data)}
</div>`;
}
...
function mLabelFromUrl(url) {
  try {
    ...
    const pub = {
      ...
      'bls.gov': 'U.S. Bureau of Labor Statistics', 'fred.stlouisfed.org': 'FRED',
    };
...
// Centered brand sign-off at the end of the post.
function mBrandFooter() {
  return `<div style="text-align:center;padding:22px 0 6px;">
  <div style="width:44px;height:2px;background:${M.gold};margin:0 auto 12px;"></div>
  <div style="font-family:${M_SERIF};font-size:15px;font-weight:600;color:${M.ink};letter-spacing:.04em;">REALTY EXPERTS®</div>
  <div style="font-family:${M_SERIF};font-size:11.5px;font-style:italic;color:${M.soft};margin-top:3px;">"Our Experience is the Difference"</div>
  <div style="font-family:${M_SANS};font-size:12px;margin-top:8px;"><a href="https://TeamRealtyExperts.com" target="_blank" rel="noopener noreferrer" style="color:${M.goldDark};text-decoration:none;font-weight:700;">TeamRealtyExperts.com</a></div>
</div>`;
}
...
function mRealEstate(data) {
  ...
  return mCard('01', 'REAL ESTATE', M.re, inner);
}
function mStocks(data) {
  ...
  return mCard('02', 'STOCKS', M.stocks, inner);
}
function mEconomy(data) {
  ...
  // Themed editions may carry Economy prose with no stat cards. Normal runs
  // always supply all three, so the filter is inert there.
  let inner = pairs.length ? mGrid(pairs) : '';
  ...
  return mCard('03', 'ECONOMY', M.economy, inner);
}
function mCrypto(data) {
  ...
  return mCard('04', 'CRYPTO', M.crypto, inner);
}
